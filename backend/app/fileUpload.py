...
load_dotenv()
AES_KEY = bytes.fromhex(os.getenv("AES_KEY"))  # Ensure AES_KEY is properly formatted
...
def upload_json_to_ipfs(data: dict) -> str:
    ...
    # Write encrypted data to stdin and pipe to 'ipfs add'
    result = subprocess.run(
        ['ipfs', 'add', '-Q', '--pin=true'],
    ...
    return result.stdout.strip()  # Returns the CID


# def uploadImageIPFS(image):
#     fileBytes = image.file.read()
#     encrypted_bytes = encrypt_file(fileBytes)
...
#         for cid in cidLis:
#             try:
#                 # Use subprocess to retrieve file
#                 result = subprocess.run(
#                     ['ipfs', 'cat', cid],
#                     capture_output=True,
#                     check=True
